...
MODEL_DIR = 'model_layer_seed' + str(MY_RANDOM_STATE)
...
class EnhancerDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        values = np.concatenate((values, values_one_mer, values_two_mer, values_three_mer), axis=0)
        # values_four_mer = self.extract_4_mer(sample)
        # values = np.concatenate((values, values_four_mer), axis=0)
        # values = np.concatenate((values, values_one_mer, values_two_mer, values_three_mer, values_four_mer), axis=0)
            ...
            for secondCh in ['A', 'C', 'G', 'T']:
                for thirdCh in ['A', 'C', 'G', 'T']:
                    for fourthCh in ['A', 'C', 'G', 'T']:
                        ...
                        my_count[four_mer] = 0.0
        for i in range(SAMPLE_LENGTH - 3):
            four_mer = sample[i:i+4]
            #print("four_mer: ", four_mer)
            my_count[four_mer] += 1
                    
        values = np.zeros((1, SAMPLE_LENGTH))
        for i in range(1,SAMPLE_LENGTH-3):
            four_mer = sample[i-1:i+3]
            values[0, i] = my_count[four_mer] / SAMPLE_LENGTH;
...
def evaluate(file_model, loader):
    ...
    # 保存acc、sensitivity、specificity、mcc、auc到文件result.txt
    with open('result.txt', 'a') as f:
        f.write(str(acc) + '\t' + str(sensitivity) + '\t' + str(specificity) + '\t' + str(mcc) + '\t' + str(auc) + '\n')
    return result
...
def prepare_test_data():
    print("\n ==> Loading test set")
    data_enhancer = read_test_file('../final_data/enhancer_test.fa')
    print("data_enhancer: ", len(data_enhancer))
    
    data_non_enhancer = read_test_file('../final_data/nonenhancer_test.fa')
...
def testing():
    ...
    for model_fn in list_model_fn:
        ...
        fpr, tpr, _ = roc_curve(y_true, result['arr_prob'])
        fpr_dict[f'Model{len(fpr_dict)+1}'] = fpr
        tpr_dict[f'Model{len(tpr_dict)+1}'] = tpr
    ...
    fpr_dict['Ensemble Model'] = fpr
    tpr_dict['Ensemble Model'] = tpr

    # 绘制ROC曲线
    plt.figure(dpi=1200)
    for i in fpr_dict:
        if i == 'Ensemble Model':
            plt.plot(fpr_dict[i], tpr_dict[i], label=i, color='black') 
        else:
    ...
    # 将data,label,y_pred_ensemble保存为csv文件
    with open(testset_file, mode='w') as outfile:
        outfile = csv.writer(outfile, delimiter=',')
        outfile.writerow(['data', 'label', 'y_pred_ensemble', 'y_prob_ensemble'])
        for i in range(len(testset["data"])):
            outfile.writerow([testset["data"][i], int(testset["label"][i]), y_pred_ensemble[i], y_prob_ensemble[i]])
